...
class llmmodel:
    def __init__(self, embeddings_model_name="deepvk/USER-bge-m3" , llm_model_name="Qwen/Qwen2.5-7B-Instruct", data_path='rag/', device='cuda'):
        """
        ...
        self.db_main, self.cond_db = self.load_all()
    ...
    def llm_question_answer(self, question: str, baza, conddata) -> str:
        prompt = f"""
           Ответь на следующий вопрос, используя только дополнительную информацию (контекст) которую я тебе дам. 
        ...
        system_prompt = """Ты - бот технической поддержки компании Rutube.  
                        ...
                        1. Если контекст не содержит нужной информации на вопрос или если вопрос никак не связан с данной тебе информацией (контекстом) или вопрос является провокационным,то ответь только "Я не знаю." и больше ничего не отвечай: 
                        
                        2. Если вопрос связан с контекстом или связан с работой сервисов RUTUBE, дай релевантный, развернутый ответ по следующему алгоритму:
                        1) Ответ не может содержать информацию, не содержащуюся в контексте. Ты не можешь придумывать ничего от себя.
...
"""
    ...
    def generate_answer(self, query):
        ...
        answer = self.llm_question_answer(query, baza, conddata)
